...
var Resource = _interopRequireWildcard(_axios);
...
// Core context
Resource.defaults.baseURL = 'http://localhost:8080/0.2.0';
_vue2.default.prototype.$http = Resource;
...
var app = new _vue2.default({
	...
	'methods': {
		...
		'login': function login(_ref2) {
			...
			var credentials = new _buffer.Buffer(username + ':' + password).toString('base64');
			var headers = {
				'Authorization': 'Basic ' + credentials,
				'X-Requested-With': 'XMLHttpRequest'
			};

			var request = this.$http.get('/auth/users/' + username + '/', { headers: headers });
			request.then(function (response) {
				...
				if (error.response) {
					...
					// Check status code and emit an adequate response
					switch (error.response.status) {
						case 401:
						case 404:
							_this.message = 'No. empleado/contraseña inválidos.';break;
						case 500:
							_this.message = 'Ocurrió un error en el servidor. Favor de notificar a un administrador.';break;
					}
				}
				// There was a connection error
				else _this.message = 'Ha ocurrido un error de conexión. Intente de nuevo más tarde.';

				if (_this.message) setTimeout(function () {
					_this.message = false;
				}, 5000);
			});
		}
